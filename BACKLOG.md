...
* **Status**: Complete — implemented via explicit artifact handoff in `plans/003-explicit-pipeline-artifacts.md`
* **Target Files**: `src/nba_optimizer/engine.py`, `src/nba_optimizer/ranker.py`, `src/nba_optimizer/exporter.py`, `src/nba_optimizer/orchestrator.py`
...
* **Acceptance Criteria**:
  * [x] Each stage `run()` function returns its output file path.
  * [x] The orchestrator captures each stage's return value and passes it explicitly to the next stage, eliminating latest-file lookup in orchestrated runs.
  * [x] Standalone module invocations (direct `python -m nba_optimizer.<module>`) still fall back to latest-file discovery when no explicit path is provided.
* **Remaining gap**: Row-count or integrity validation for standalone usage; run-ID linking is not implemented (the explicit handoff approach was sufficient). 
